...
const MyOrder = props => {
  ...
  const handleRenderOrderInfo = () => {
    if (
      Object.keys(storeOrder.orderInfo).length &&
      ...
      htmlCartInfo.push(
        <Descriptions key="total">
          ...
          {status === ORDER_STATUS.SERVED && (
            <Descriptions.Item>
              <Button type="primary" onClick={() => history.push('/rating')}>Rate this order</Button>
            </Descriptions.Item>
          )}
        </Descriptions>,
      );
